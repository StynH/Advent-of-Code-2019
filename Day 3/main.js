class Path {
    ...
    static calculateManhattanDistance(pointOne, pointTwo){
        return Math.abs(pointTwo.x - pointOne.x) + Math.abs(pointTwo.y - pointOne.y);
    }

    findIntersections(rhs){
        let intersections = [];
        //Awful, but can't come up with anything else right now.
        for(let x = 0; x < this.path.length; ++x){
            if(this.path[x] !== undefined){
                for(let y = 0; y < this.path[x].length; ++y){
                    //If intersection is found.
                    if(this.path[x][y] === true && rhs.path[x] !== undefined && rhs.path[x][y] === true){
    ...
    addRangeToPath(pair){
        ...
        for(let coordX of _.range(0, x, xIt)){
            ...
            if(this.path[this.lastX] === undefined){
                this.path[this.lastX] = [];
            }

            this.path[this.lastX][this.lastY] = true;
        }
...
$( document ).ready(function() {
    ...
    //Split paths into direction and distance.
    const splitPathOne = _.map(pathDataOne, (val) => {
        return { dir: val.substring(0, 1),num: val.substring(1) };
    });

    const splitPathTwo= _.map(pathDataTwo, (val) => {
        return { dir: val.substring(0, 1),num: val.substring(1) };
    });

    //Create our 2 paths.
    let pathOne = new Path();
    let pathTwo = new Path();

    //Add all pieces.
    _.each(splitPathOne, (piece) =>{
        pathOne.addRangeToPath(piece);
    });

    _.each(splitPathTwo, (piece) =>{
        pathTwo.addRangeToPath(piece);
    });

    //The origin.
    const origin = { x: 0, y: 0 };

    //Get all intersections, map to manhattan distance and sort low to high.
    const intersections = _.chain(pathOne.findIntersections(pathTwo)).map((intersection) => {
        return Path.calculateManhattanDistance(origin, intersection);
    }).sortBy((distance) => {
        return distance;
    }).value();

    console.log("Shortest distance: " + intersections[0]);
});